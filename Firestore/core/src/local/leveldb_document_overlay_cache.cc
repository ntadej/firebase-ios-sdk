...
LevelDbDocumentOverlayCache::GetOverlays(absl::string_view collection_group,
  ...
  ForEachKeyInCollectionGroup(
      ...
      [&](LevelDbDocumentOverlayKey&& key) -> bool {
...
void LevelDbDocumentOverlayCache::SaveOverlay(int largest_batch_id,
  ...
  transaction->Put(LevelDbDocumentOverlayCollectionGroupIndexKey::Key(key), "");
}
...
void LevelDbDocumentOverlayCache::DeleteOverlay(
  ...
  transaction->Delete(LevelDbDocumentOverlayCollectionGroupIndexKey::Key(key));
}
...
void LevelDbDocumentOverlayCache::ForEachKeyInCollectionGroup(
    ...
    int since_batch_id,
    std::function<bool(LevelDbDocumentOverlayKey&&)> callback) const {
  const std::string index_start_key =
  ...
  for (it->Seek(index_start_key);
    ...
    bool keep_going = callback(std::move(key).ToLevelDbDocumentOverlayKey());
    if (!keep_going) {
      break;
    }
  }
